...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            # input 1824
            nn.Conv1d(1, 64, kernel_size=2, stride=2, padding=1, bias=False),
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # state size 114
            nn.Conv1d(512, 1, kernel_size=65, stride=1, padding=0, bias=False),
            nn.Sigmoid()
        )
...
class Generator(nn.Module):
    def __init__(self, nz):
        super().__init__()
        self.main = nn.Sequential(
            nn.ConvTranspose1d(nz, 512, 131, 1, 0, bias=False),
            nn.BatchNorm1d(512),
            nn.ReLU(True),
            ...
            nn.ConvTranspose1d(64, 1, 4, 1, 1, bias=False),
            nn.Tanh()
        )
